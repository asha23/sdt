...
//******************************************************************************
// REGISTER SOME USEFUL CUSTOM POST TYPES
//******************************************************************************
...
function gallery_post_type() {
	...
	$labels = array(
		...
		'singular_name'         => _x( 'Galleries', 'Post Type Singular Name', 'text_domain' ),
...
function staff_posts() {
	$labels = array(
  	  ...
  	  'parent_item_colon'     => __( 'Staff Item:', 'text_domain' ),
...
add_action( 'init', 'staff_posts' );


function staff_taxonomy() {
  $labels = array(
    'name'              => _x( 'Staff Types', 'taxonomy general name' ),
    'singular_name'     => _x( 'Staff Types', 'taxonomy singular name' ),
...
// Add some terms on install

function add_default_terms() {
	$galleryTerms = get_terms('galleries_category', array('hide_empty' => 'false'));

	if(empty($galleryTerms)) { // Add some terms

		wp_insert_term('Video', 'galleries_category', array('slug' => 'video'));
		wp_insert_term('Image', 'galleries_category', array('slug' => 'image'));
	}

	$staffTerms = get_terms('staff_category', array('hide_empty' => 'false'));

	if(empty($staffTerms)) { // Add some terms
		wp_insert_term('Cast', 'staff_category', array('slug' => 'cast'));
		wp_insert_term('Creative', 'staff_category', array('slug' => 'creative'));
	}
}
